...
class NyxProgressBar : ConstraintLayout {
    ...
    init {
        ...
        val calculatedPaddingHeight =
            nyx_progress_bar.minimumHeight - (nyx_progress_bar.minimumHeight / sqrt(2.0)).toInt()
        val calculatedPaddingWidth = nyx_progress_bar.minimumWidth - (nyx_progress_bar.minimumWidth / sqrt(2.0)).toInt()
        nyx_progress_bar.setPadding(
            calculatedPaddingWidth,
            calculatedPaddingHeight,
            calculatedPaddingWidth,
            calculatedPaddingHeight
        )
    }
        ...
        context.theme.obtainStyledAttributes(
            ...
            try {
                when (getInt(R.styleable.Accent_accent, 0)) {
                    0 -> nyx_progress_bar.indeterminateDrawable =
                        context.getDrawable(R.drawable.nyx_spinning_logo_member)
                    1 -> nyx_progress_bar.indeterminateDrawable =
                        context.getDrawable(R.drawable.nyx_spinning_logo_employee)
                }
            } finally {
                recycle()
            }
